...
from utils import delegable_nodes, get_delegate_list_for_node


class Action(User):

    def __init__(self, web3: Web3, total_account: int):
        super().__init__(web3, total_account)
    ...
    def delegate(self, account):
        ...
        result = self.ppos.delegate(balance_type, node['NodeId'], amount, account.private_key, {"nonce": account.nonce})
    ...
    def undelegate(self, account):
        ...
        result = self.ppos.withdrewDelegate(block_number, node_id, amount, account.private_key, {"nonce": account.nonce})
    ...
    def withdraw_reward(self, account):
        ...
        result = self.ppos.withdrawDelegateReward(account.private_key, {"nonce": account.nonce})
