...
restricting(main_private_key, staking_address, plan)
staking(staking_private_key, 0, node_url, 20000 * 10 ** 18, 1000)
# 增持质押
...
plan = [{'Epoch': 1, 'Amount': 80 * 10 ** 18}]
restricting(main_private_key, delegation_address, plan)
...
version_proposal(staking_private_key, node_id, upgrade_version, upgrade_voting_rounds)
...
for node_url, private_key in node_list:
    try:
        vote(private_key, node_url, proposal_id, 1)
